...
class LlamaLocalPlanningApi(LLAMPApi):
    ...
    def _query_actions(self, goal, world, world_args, temperature: float = 0.0, first_query_kwargs=dict(),
        ...
        prompt_english_to_actions = build_user_msg_nl(contexts=self.llm.rag_manager.query_all_dbs(goal)).format(
            **world_args)
        ...
        if actions_english is None or reprompted:
            ...
            actions_english = self.ask(prompt_english_to_actions, prompt_name=first_prompt_name,
            ...
            if actions_string is None or reprompted:
                actions_string = self.ask(prompt_actions_to_fl, prompt_name=f'actions_string{self.suffix}',
                                          temperature=temperature,
