
(define
  (problem test_kitchen_chicken_soup_250906_120609_seed_908633)
  (:domain domain)

  (:objects
	base
	base-torso
	basin#1
	basin#1::basin_bottom
	braiserbody#1
	braiserbody#1::braiser_bottom
	braiserlid#1
	chicken-leg
	counter#1
	counter#1::chewie_door_left_joint
	counter#1::chewie_door_right_joint
	counter#1::front_left_stove
	counter#1::front_right_stove
	counter#1::hitman_countertop
	counter#1::indigo_tmp
	counter#1::sektion
	faucet#1
	faucet#1::joint_faucet_0
	fridge#1
	fridge#1::fridge_door
	fridge#1::shelf_top
	head
	left_arm
	left_gripper
	oven#1
	oven#1::knob_joint_2
	oven#1::knob_joint_3
	pepper-shaker
	right_arm
	right_gripper
	salt-shaker
	torso
  )

  (:init
	;; discrete facts (e.g. types, affordances)
	(canmove)
	(canpick)

	(arm left)
	(arm right)

	(canmovebase)

	(canpull left)
	(canpull right)

	(cangrasphandle)
	(handempty left)
	(handempty right)

	(food chicken-leg)

	(controllable left)
	(controllable right)

	(region braiserbody#1)
	(region fridge#1::shelf_top)
	(region counter#1::front_left_stove)
	(region basin#1::basin_bottom)
	(region counter#1::hitman_countertop)
	(region braiserbody#1::braiser_bottom)
	(region counter#1::sektion)
	(region counter#1::indigo_tmp)
	(region counter#1::front_right_stove)

	(graspable chicken-leg)
	(graspable salt-shaker)
	(graspable braiserbody#1)
	(graspable braiserlid#1)
	(graspable pepper-shaker)

	(sprinkler pepper-shaker)
	(sprinkler salt-shaker)

	(space counter#1::sektion)
	(space braiserbody#1)

	(surface fridge#1::shelf_top)
	(surface counter#1::front_left_stove)
	(surface braiserbody#1)
	(surface braiserbody#1::braiser_bottom)
	(surface basin#1::basin_bottom)
	(surface counter#1::hitman_countertop)
	(surface counter#1::indigo_tmp)
	(surface counter#1::front_right_stove)

	(bconf q0=(2.0, 6.25, 0.2, 3.142))
	(staticlink counter#1::indigo_tmp)
	(staticlink braiserbody#1)
	(staticlink fridge#1::shelf_top)
	(staticlink counter#1::front_left_stove)
	(staticlink braiserbody#1::braiser_bottom)
	(staticlink basin#1::basin_bottom)
	(staticlink counter#1::hitman_countertop)
	(staticlink counter#1::front_right_stove)
	(staticlink counter#1::sektion)

	(atbconf q0=(2.0, 6.25, 0.2, 3.142))

	(unattachedjoint oven#1::knob_joint_3)
	(unattachedjoint counter#1::chewie_door_left_joint)
	(unattachedjoint counter#1::chewie_door_right_joint)
	(unattachedjoint faucet#1::joint_faucet_0)
	(unattachedjoint fridge#1::fridge_door)
	(unattachedjoint oven#1::knob_joint_2)

	(door counter#1::chewie_door_left_joint)
	(door counter#1::chewie_door_right_joint)
	(door fridge#1::fridge_door)

	(isjointto fridge#1::fridge_door fridge#1)
	(isjointto faucet#1::joint_faucet_0 faucet#1)
	(isjointto oven#1::knob_joint_3 oven#1)
	(isjointto counter#1::chewie_door_right_joint counter#1)
	(isjointto counter#1::chewie_door_left_joint counter#1)
	(isjointto oven#1::knob_joint_2 oven#1)
	(joint counter#1::chewie_door_right_joint)
	(joint oven#1::knob_joint_3)
	(joint faucet#1::joint_faucet_0)
	(joint fridge#1::fridge_door)
	(joint oven#1::knob_joint_2)
	(joint counter#1::chewie_door_left_joint)

	(containable chicken-leg counter#1::sektion)
	(containable braiserbody#1 counter#1::sektion)
	(containable salt-shaker counter#1::sektion)
	(containable chicken-leg braiserbody#1)
	(containable braiserlid#1 counter#1::sektion)
	(containable pepper-shaker counter#1::sektion)
	(position oven#1::knob_joint_2 pstn5641=0.0)
	(position oven#1::knob_joint_3 pstn5642=0.0)
	(position counter#1::chewie_door_right_joint pstn5638=1.872)
	(position faucet#1::joint_faucet_0 pstn5643=0.0)
	(position counter#1::chewie_door_left_joint pstn5639=-1.872)
	(position fridge#1::fridge_door pstn5640=1.78)

	(stackable salt-shaker counter#1::indigo_tmp)
	(stackable pepper-shaker counter#1::hitman_countertop)
	(stackable braiserlid#1 counter#1::indigo_tmp)
	(stackable chicken-leg basin#1::basin_bottom)
	(stackable pepper-shaker counter#1::indigo_tmp)
	(stackable braiserlid#1 counter#1::front_left_stove)
	(stackable chicken-leg braiserbody#1::braiser_bottom)
	(stackable braiserlid#1 braiserbody#1)
	(stackable salt-shaker counter#1::hitman_countertop)
	(stackable braiserlid#1 basin#1::basin_bottom)
	(stackable braiserlid#1 counter#1::hitman_countertop)
	(stackable chicken-leg counter#1::indigo_tmp)
	(stackable braiserbody#1 counter#1::indigo_tmp)

	(atposition oven#1::knob_joint_2 pstn5641=0.0)
	(atposition fridge#1::fridge_door pstn5640=1.78)
	(atposition counter#1::chewie_door_left_joint pstn5639=-1.872)
	(atposition counter#1::chewie_door_right_joint pstn5638=1.872)
	(atposition oven#1::knob_joint_3 pstn5642=0.0)
	(atposition faucet#1::joint_faucet_0 pstn5643=0.0)

	(isclosedposition oven#1::knob_joint_2 pstn5641=0.0)
	(isclosedposition oven#1::knob_joint_3 pstn5642=0.0)
	(isclosedposition faucet#1::joint_faucet_0 pstn5643=0.0)

	(pose braiserlid#1 p7724=(0.567, 7.872, 0.712, 0.0, -0.0, 1.944))
	(pose braiserbody#1 p7722=(0.7, 8.745, 0.923, 0.0, -0.0, 1.571))
	(pose pepper-shaker p7727=(0.764, 7.303, 1.164, 0.0, -0.0, 3.142))
	(pose chicken-leg p7723=(0.654, 4.846, 1.384, 0.0, 0.0, -0.366))
	(pose salt-shaker p7726=(0.771, 7.071, 1.152, 0.0, -0.0, 3.142))

	(atpose braiserlid#1 p7724=(0.567, 7.872, 0.712, 0.0, -0.0, 1.944))
	(atpose braiserbody#1 p7722=(0.7, 8.745, 0.923, 0.0, -0.0, 1.571))
	(atpose pepper-shaker p7727=(0.764, 7.303, 1.164, 0.0, -0.0, 3.142))
	(atpose chicken-leg p7723=(0.654, 4.846, 1.384, 0.0, 0.0, -0.366))
	(atpose salt-shaker p7726=(0.771, 7.071, 1.152, 0.0, -0.0, 3.142))

	(isopenedposition counter#1::chewie_door_right_joint pstn5638=1.872)
	(isopenedposition fridge#1::fridge_door pstn5640=1.78)
	(isopenedposition counter#1::chewie_door_left_joint pstn5639=-1.872)

	(ataconf left aq896=(0.677, -0.343, 1.2, -1.467, 1.242, -1.954, 2.223))
	(ataconf right aq648=(-0.677, -0.343, -1.2, -1.467, -1.242, -1.954, -2.223))

	(aconf right aq648=(-0.677, -0.343, -1.2, -1.467, -1.242, -1.954, -2.223))
	(aconf left aq896=(0.677, -0.343, 1.2, -1.467, 1.242, -1.954, 2.223))

	(contained pepper-shaker p7727=(0.764, 7.303, 1.164, 0.0, -0.0, 3.142) counter#1::sektion)
	(contained salt-shaker p7726=(0.771, 7.071, 1.152, 0.0, -0.0, 3.142) counter#1::sektion)

	(supported braiserlid#1 p7724=(0.567, 7.872, 0.712, 0.0, -0.0, 1.944) counter#1::front_left_stove)
	(supported braiserbody#1 p7725=(0.7, 8.745, 0.923, 0.0, -0.0, 1.571) counter#1::indigo_tmp)

  )

  (:goal (and
    (open fridge#1::fridge_door)
  ))
)
        